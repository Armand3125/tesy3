...
if uploaded_image is not None:
    ...
    # Traitement KMeans avec la couleur noire par défaut
    img_arr = np.array(resized_image)
    pixels = img_arr.reshape(-1, 3)

    # Nombre de couleurs par défaut (noir)
    num_selections = 4
    selected_colors = [pal["NC"]] * num_selections  # Initialiser avec les noirs par défaut
    ...
    # Afficher l'image traitée par KMeans avec les noirs comme couleur par défaut
    ...
    # Appliquer KMeans à nouveau avec les nouvelles couleurs sélectionnées
    kmeans = KMeans(n_clusters=num_selections, random_state=0).fit(pixels)
    labels = kmeans.labels_
    ...
    # Afficher l'image après modification des couleurs par l'utilisateur
    st.image(new_image, caption=f"Image après modification ({num_selections} couleurs)", use_column_width=False)
